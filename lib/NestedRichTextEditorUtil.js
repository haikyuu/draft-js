  ...
  onTab: function onTab(event, editorState, maxDepth) {
    ...
    if (!event.shiftKey) {
      ...
      if (childWasUntabbed && parentKey != null) {
        ...
        var _prevSiblingKey = _parent2 != null // parent may have been deleted
        ? _parent2.getPrevSiblingKey() : null;
        if (_prevSiblingKey != null && _parent2.getChildKeys().count() > 0) {
          var _prevSibling = blockMap.get(_prevSiblingKey);
          if (_prevSibling != null && _prevSibling.getChildKeys().count() > 0) {
            blockMap = DraftTreeOperations.mergeBlocks(blockMap, _prevSiblingKey);
          }
        }
      }
    }
