...
var mergeBlocks = function mergeBlocks(blockMap, key) {
  ...
  var nextChildren = childKeys.map(function (k, i) {
    return blockMap.get(k).merge({
      parent: key,
  ...
  var newBlockMap = blocks.takeUntil(function (b) {
    return b.getKey() === key;
  }).concat([[block.getKey(), block]], nextChildren.map(function (b) {
    return [b.getKey(), b];
  }), nextNextBlockKey != null ? [[nextNextBlockKey, blockMap.get(nextNextBlockKey).merge({ prevSibling: key })]] : [], blocks.skipUntil(function (b) {
    return b.getKey() === nextNextBlockKey;
  }).rest()).toOrderedMap();
  verifyTree(newBlockMap);
  return newBlockMap;
};
