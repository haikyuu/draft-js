...
var isValidAnchor = function isValidAnchor(node) {
  if (isHTMLAnchorElement(node)) {
    var castedNode = node;
    return castedNode.href && (castedNode.protocol === 'http:' || castedNode.protocol === 'https:' || castedNode.protocol === 'mailto:');
  }
  return false;
};
...
var isValidImage = function isValidImage(node) {
  if (isHTMLImageElement(node)) {
    var castedNode = node;
    return !!(castedNode.attributes.getNamedItem('src') && castedNode.attributes.getNamedItem('src').value);
  }
  return false;
};
